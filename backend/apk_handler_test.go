...
func TestAPKDownload(t *testing.T) {
	...
	request := httptest.NewRequest(http.MethodGet, "/downloads/fitness-tracker.apk", nil)
...
func TestAPKDownloadRejectsUnsupportedMethod(t *testing.T) {
	recorder := httptest.NewRecorder()
	request := httptest.NewRequest(http.MethodPost, "/downloads/fitness-tracker.apk", nil)

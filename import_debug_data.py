...
while i < len(lines):
    ...
    if line.startswith('•'):
        ...
        if match:
            ...
            # 如果没有明确的组数，检查下一行是否是相同的动作
            if not sets_match:
                j = i + 1
                while j < len(lines) and lines[j].strip().startswith('•'):
                    next_match = re.match(r'•(.+?)：(.+)', lines[j].strip())
                    if next_match and next_match.group(1).strip() == exercise_name:
                        sets += 1
                        j += 1
                    else:
                        break
    ...
    for exercise_session in exercise_sessions:
        ...
        for set_num in range(1, sets + 1):
            training_records.append({
                ...
                'setNumber': set_num,
                'weight': exercise_session['weight'],
                'reps': exercise_session['reps'],
...
for session in sorted(training_sessions, key=lambda x: x['date']):
    print(f"  {session['date']} - {list(groups_dict.keys())[session['groupId']-1]}")
